...
from muse_dictionary import Dictionary
...
def initialize_exp(params):
    ...
    print('The experiment will be stored in %s' % params.exp_path)
    return ""
...
def read_txt_embeddings(params, target, full_vocab):
    """
    ...
    lang = params['target_lang'] if target else params['related_lang']
    emb_path = params['target_emb'] if target else params['related_emb']
    _emb_dim_file = params['word_dim']
                ...
                if word in word2id:
                    if full_vocab:
                        print("Word '%s' found twice in %s embedding file"
                              % (word, 'target' if target else 'related'))
                else:
                    if not vect.shape == (_emb_dim_file,):
                        print("Invalid dimension (%i) for %s word '%s' in line %i."
                              % (vect.shape[0], 'target' if target else 'related', word, i))
                        continue
    ...
    embeddings = embeddings.cuda() if (params['gpu'] and not full_vocab) else embeddings

    assert embeddings.size() == (len(dico), params['word_dim'])
...
def load_embeddings(params, target, full_vocab=False):
    """
    ...
    """
    assert type(target) is bool and type(full_vocab) is bool
    return read_txt_embeddings(params, target, full_vocab)
